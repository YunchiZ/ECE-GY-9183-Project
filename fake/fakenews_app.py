...
def get_next_model_version(base_dir="model"):
    ...
    existing_versions = [
        int(p.name.replace("Xlnet-v", ""))
        for p in base_path.iterdir()
        if p.is_dir() and p.name.startswith("Xlnet-v") and p.name.replace("Xlnet-v", "").isdigit()
    ]
    next_version = max(existing_versions + [-1]) + 1  # 如果不存在则为 v0
    return f"Xlnet-v{next_version}", base_path / f"Xlnet-v{next_version}"
