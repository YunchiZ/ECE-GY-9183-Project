...
data_quality_missing_values = Gauge(
    "data_quality_missing_values", "Number of missing values", ["task", "field"]
)
data_quality_duplicates = Gauge(
    "data_quality_duplicates", "Number of duplicate entries", ["field"]
)
data_quality_processing_errors = Counter(
    "data_quality_processing_errors", "Number of data processing errors", ["error_type"]
)
    ...
    try:
        ...
        for field in fields_to_check:
            if field in data.columns:
                ...
                if missing_count > 0:
                    data_quality_missing_values.labels(
                        task=task_label, field=field
                    ).inc(missing_count)
        ...
        data_quality_processing_errors.labels(
            error_type="quality_check_and_clean_error"
        ).inc()
        logging.error(
            f"Error checking data quality and clean for {table_name}: {str(e)}"
        )
        return None
...
def save_to_volume(data, table_name):
    ...
    try:
        ...
        logging.info(
            f"Data saved to {file_path}, waiting for rclone to upload to object storage"
        )
        return True
        ...
        try:
            ...
            for table_name, table_data in evaluation_data.items():
                ...
                if processed_data is None:
                    logging.error(
                        f"Failed to process evaluation data for table {table_name}"
                    )
                    continue
                ...
                try:
                    ...
                    if train_response.status_code == 200:
                        ...
                        if retry_count < max_retries:
                            logging.warning(
                                f"Failed to trigger train service (attempt {retry_count}/{max_retries}): {train_response.text}"
                            )
                        else:
                            logging.error(
                                f"Failed to trigger train service after {max_retries} attempts: {train_response.text}"
                            )
                except Exception as e:
                    retry_count += 1
                    if retry_count < max_retries:
                        logging.warning(
                            f"Error triggering train service (attempt {retry_count}/{max_retries}): {str(e)}"
                        )
                    else:
                        logging.error(
                            f"Error triggering train service after {max_retries} attempts: {str(e)}"
                        )
...
def metrics():
    ...
    )  # text specifies return type as plain text
