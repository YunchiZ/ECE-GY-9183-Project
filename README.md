...
The deployment status `dployment_stage` mentioned above has three states in the thread:
+`normal`: Only the officially serving models make predictions and return results. At this time, there are no candidate models that need to be tested.
+`shadow`: The `candidate` models have passed offline tests and need to be evaluated online in the current container (shadow test). In this mode, the API thread first uses the `serving` model to make predictions and promptly provides the results to the *Frontend*, and then uses the `candidate` model to make predictions. However, the latter's results are not provided to the user and only retained in the deploy data volume for `monitor` analysis and evaluation.
+`canary`: The `candidate` model has passed online evaluations and need to gradually expand their deployment scale. In this mode, the API needs to select whether to return the prediction values of the serving model or the candidate model based on the probability value *p*, which is also called traffic diversion. This probability value will gradually increase from 0.2, 0.4 to 1 with the increase of API call times (every 100 times). When *p* increases to 1, it means that the new model will completely replace the old model. At this time, the currently operational model `serving` will be changed to `abandon`; the candidate model will be changed to `serving`, and the database of this model will be reset. Finally, the `monitor` docker will be notified through HTTP to terminate the previous result detection and start long-term monitoring of the newly deployed model.
