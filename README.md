...
#### 1). Model serving - API
When requested by the user, the port 8000 `/predict` route of *deploy* docker is first triggered by *Frontend* docker, and the `serving` model weights (including BART, DistilBERT, and XLNet) are called to make the prediction. This process is communicated to the Prometheus container on docker port 8001, and the results are not only returned to the front-end interface, but also logged into the deploy data volume for the monitor container to match, analyze, and migrate.
